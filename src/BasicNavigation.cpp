...
void MotorController::ChangeDirection(DeltaDir deltadir, Speed speeddiff){
    ...
    direction = abs(direction + deltadir) & 0xff;  // update the direction so that its mag is not above 127
    ...
    ohmega = (((int)speeddiff << 8)) / (M_PI *  WHEELBASE));  // get tangential speed and convert it to angular

    waittime = milliseconds((uint64_t)(1000*abs(deltadir)/ohmega));   // calculate wait time in millis

    busy = true;
    
    hal.motor[LEFT].SetSpeed( (deltadir > 0) ? currspeed - speeddiff: currspeed + speeddiff);  // give the motors their new speeds
    hal.motor[RIGHT].SetSpeed( (deltadir > 0) ? currspeed + speeddiff: currspeed - speeddiff);

    this_thread::sleep_for(waittime);  // wait until the turn has covered the delta

    hal.motor[LEFT].SetSpeed(currspeed);  // reset the motors so that the 
